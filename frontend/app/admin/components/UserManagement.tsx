...
const UserManagement = () => {
  ...
  const toggleUserVerification = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      setToastMessage({
        type: 'success',
        message: `User ${currentStatus ? 'unverified' : 'verified'} successfully`
      });
  ...
  return (
    <div className="bg-[#E8EDE1] border-4 border-[#556B2F] p-6 shadow-brutal">
      <h2 className="text-3xl font-black mb-6 text-[#2F4F4F]">User Management</h2>
        ...
        <table className="w-full bg-white border-2 border-[#556B2F]">
          <thead>
            <tr className="bg-[#D2E3C8]">
              <th className="p-3 text-left font-bold text-[#2F4F4F] border-b-2 border-[#556B2F]">User</th>
              <th className="p-3 text-left font-bold text-[#2F4F4F] border-b-2 border-[#556B2F]">Type</th>
              <th className="p-3 text-left font-bold text-[#2F4F4F] border-b-2 border-[#556B2F]">Status</th>
              <th className="p-3 text-left font-bold text-[#2F4F4F] border-b-2 border-[#556B2F]">Joined</th>
          ...
          <tbody>
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((user) => (
                <tr key={user.id} className="border-b border-[#556B2F] hover:bg-[#F5F7F2]">
                  <td className="p-3">
                    <div>
                  ...
                  <td className="p-3 text-[#2F4F4F]">{formatDate(user.createdAt)}</td>
                  <td className="p-3">
                    <button
