/**
 * Direct test script for document upload to backend
 */
const axios = require('axios');
const fs = require('fs');
const FormData = require('form-data');
const path = require('path');
const dotenv = require('dotenv');

// Load environment variables from .env file in the root directory
dotenv.config({ path: path.resolve(__dirname, '../.env') });

// Configuration
const apiUrl = 'http://localhost:8080/api';

// Test wallet address
const walletAddress = '0x1234567890123456789012345678901234567890';

// Test token - replace with a valid token from your backend logs
const token = 'eyJhbGciOiJSUzI1NiIsImtpZCI6Ijg1NzA4MWNhOWNiYjM3YzIzNDk4ZGQzOTQzYmYzNzFhMDU4ODNkMjgiLCJ0eXAiOiJKV1QifQ..RKQxLPR2n42aiJpkUfZ-4mCtLf8QBNctu-kTiBcNcGleiqtSDzFrPR2EWcLFX5EsbB5G3IWIEQ-j92K8epa-TQVvnau5cLYy4dxo8HtWV18_uH0Ll2OIwjwezAnuSGs7lqRvgMTewqbzAJ87JX8P7sXnabIsuviRpMUhH-uKoiV9ALMM1qWlMBGzME3iwJFjHSst34W5DOk8A3F1ikTG67QSLtT73pVVHK20xeBOFyjn0M4tcAFNT8TGK2hqLO5BhHqps7AQ0e1wjPzUZLF3ZY9VDSo40CKIpn1dT6rbCMm0wfix3ytLX2Yz4t9-tYa2HrubCd_Y_LIApg5D1DxpPQ';

async function runTest() {
  try {
    console.log('Starting direct document upload test to backend...');
    
    // Create a small test file
    const testFilePath = './test-document.txt';
    fs.writeFileSync(testFilePath, 'This is a test document for upload testing.');
    
    // Create form data using the node-formdata library
    const form = new FormData();
    
    // Add the file as a stream
    const fileStream = fs.createReadStream(testFilePath);
    form.append('document_file', fileStream, {
      filename: 'test-document.txt',
      contentType: 'text/plain',
    });
    
    // Add other fields
    form.append('documentName', 'Test Document');
    form.append('documentType', 'identity');
    form.append('verifyingOrgId', 'org1');
    
    console.log('Form data created with the following fields:');
    for (const [key, value] of Object.entries(form)) {
      console.log(`- ${key}: ${value}`);
    }
    
    console.log('Sending request to:', `${apiUrl}/documents/upload`);
    console.log('Content-Length:', form.getLengthSync());
    
    // Upload the document directly to the backend
    const response = await axios.post(
      `${apiUrl}/documents/upload`,
      form,
      {
        headers: {
          ...form.getHeaders(), // This is important - it sets the correct Content-Type with boundary
          Authorization: `Bearer ${token}`,
        },
        // Important settings for file uploads
        maxContentLength: Infinity,
        maxBodyLength: Infinity,
      }
    );
    
    console.log('Document upload response:', response.data);
    
    // Clean up
    fs.unlinkSync(testFilePath);
    
    console.log('Test completed successfully!');
  } catch (error) {
    console.error('Test failed:', error);
    if (error.response) {
      console.error('Response data:', error.response.data);
      console.error('Response status:', error.response.status);
      console.error('Response headers:', error.response.headers);
    }
  }
}

runTest();
